...
def partition_score(intervals, time_frame, required_attacks=3):
    '''
    ...
    for i in range(len(intervals)):
        ...
        # we create and append a _new_ mini-segment if
        ...
        (len(interval['pitches']) == 0 and \
         len(mini_segments[last]['pitches']) > 0):
            ...
            mini_segments.append({'start': i, # start is the interval no. at which the mini-segment start, used only for printing
                                  'length': 1,
    ...
    while m in range(no_of_mini_segments):
        ...
        # check how many mini segments that fit within the timeframe
        ...
        # --- mini-segments now collected, add them to segment --- #
        
        if total_attacks >= required_attacks:
            # found enoughh note attacks to start new segment
            segments.append({'start': mini_seg['start'],
                             'possible_chord': True,
                             'candidates': [],
